...
st.set_page_config(page_title="Webcam Prediction", page_icon="📹", layout="wide")
...
HISTORY_DIR = os.path.join(BASE_DIR, "data", "history") # For history JSON files
WEBCAM_HISTORY_FILE_PATH = os.path.join(HISTORY_DIR, "webcam_snapshot_history.json")
...
try:
    ...
    print(f"[DEBUG-WebcamPage] Live predictions directory {LIVE_PREDICTIONS_DIR} ensured.")
    ...
    if os.path.exists(WEBCAM_HISTORY_FILE_PATH):
        try:
            with open(WEBCAM_HISTORY_FILE_PATH, 'r') as f:
                history = json.load(f)
                print(f"[DEBUG-WebcamPage] Loaded {len(history)} snapshots from history file.")
                return history
        except json.JSONDecodeError:
            print(f"[ERROR-WebcamPage] Could not decode JSON from snapshot history. Starting fresh.")
            return []
        except Exception as e:
            print(f"[ERROR-WebcamPage] Could not load snapshot history: {e}. Starting fresh.")
            return []
    return []
...
def save_snapshot_history_to_file(snapshot_data):
    try:
        ...
        print(f"[DEBUG-WebcamPage] Saved {len(snapshot_data)} snapshots to history file.")
    except Exception as e:
...
if "snapshots" not in st.session_state:
    st.session_state.snapshots = load_snapshot_history_from_file() # Load from file if empty
...
def load_models_for_webcam():
    ...
    try:
        if not os.path.exists(BBOX_MODEL_PATH):
            ...
            print(f"[ERROR-WebcamPage] BBox model not found: {BBOX_MODEL_PATH}")
            return None
        if not os.path.exists(SEG_MODEL_PATH):
            # Segmentation model might be optional for some features
            print(f"[WARNING-WebcamPage] Segmentation model not found or not configured: {SEG_MODEL_PATH}")
        ...
        predictor = LiveModelPredictor(bbox_loader, seg_loader) # seg_loader can be None
...
st.markdown("View your webcam feed and see real-time structural defect analysis. Settings are on the sidebar.")

if not st.session_state.models_loaded_webcam:
    st.sidebar.error("Models not loaded. Predictions may be limited or unavailable.")
...
with st.sidebar:
    ...
    confidence_threshold = st.slider("Confidence threshold for detections", 0.0, 1.0, 0.35, step=0.05)
    ...
    highlight_detected_defects = st.checkbox("Highlight detected defects", value=True)
    ...
    if st.session_state.confirm_delete_snapshot_history:
        st.sidebar.warning("Delete all snapshot history and associated image files? This cannot be undone.")
        if st.sidebar.button("Yes, Delete All Snapshot History", type="primary"):
            deleted_files_count = 0
            not_found_count = 0
            error_deleting_count = 0
            history_to_delete = list(st.session_state.snapshots) # Iterate over a copy
            ...
            for entry in history_to_delete:
                ...
                if path and os.path.exists(path):
                    try:
                        ...
                        print(f"[ERROR-WebcamPage] Could not delete snapshot file {path}: {e_del}")
        ...
        st.session_state.confirm_delete_snapshot_history = False # Ensure it's false if no history
...
with col1:
    ...
    camera_image_buffer = st.camera_input("Enable webcam to start analysis", key="webcam_feed")
...
if camera_image_buffer is not None and st.session_state.live_predictor:
    try:
        ...
        with col1: # Display processed image in the first column
            st.image(processed_frame_rgb, caption="Live feed with detections", use_column_width=True)
            ...
            if st.button("📸 Take Snapshot & Save Details", key="snapshot_btn"):
                ...
                try:
                    ...
                    new_snapshot_details = {
                        ...
                        "detections": list(detections_this_frame), # Ensure it's a list
                    ...
                    st.session_state.snapshots.sort(key=lambda x: x.get('timestamp_unix', 0), reverse=True)
                    ...
                    save_snapshot_history_to_file(st.session_state.snapshots) # Save updated history
                    st.success(f"Snapshot saved as {snapshot_filename} in {SNAPSHOT_DIR}")
                except Exception as e:
            ...
            if st.session_state.auto_save_live_frames and detections_this_frame:
                try:
                    ...
                    auto_save_path = os.path.join(LIVE_PREDICTIONS_DIR, auto_save_filename)
                    ...
                    print(f"[DEBUG-WebcamPage] Auto-saved frame with detections to {auto_save_path}")
                except Exception as e:
    ...
    with col1:
        st.error("Live predictor is not initialized. Cannot process webcam feed. Check model loading status.")
...
with col2:
    ...
    if camera_image_buffer is not None and st.session_state.live_predictor:
        ...
        if detections_this_frame:
            ...
            for det in detections_this_frame:
                st.markdown(f"- **{det.get('type', 'N/A')}**: {det.get('confidence', 0.0):.2f} conf.")
        else:
    ...
    if st.session_state.snapshots:
        ...
        if not snapshot_options:
            ...
            selected_snapshot_display_key = st.selectbox("View snapshot from history:", options=list(snapshot_options.keys()))
            ...
            if selected_snapshot_display_key:
                ...
                try:
                    if not os.path.exists(selected_snap_details["image_path"]):
                        st.error(f"Snapshot image not found: {selected_snap_details['image_path']}. It may have been deleted.")
                    else:
                        snap_img = Image.open(selected_snap_details["image_path"])
                        st.image(snap_img, caption=f"Snapshot: {selected_snap_details['filename']}", use_column_width=True)
                        ...
                        if selected_snap_details["detections"]:
                            for det in selected_snap_details["detections"]:
                                ...
                                st.markdown(f"- **{det.get('type', 'N/A')}**: {det.get('confidence', 0.0):.2f}{bbox_str}")
                        else:
                            st.info("No defects were recorded for this snapshot based on its settings.")
                        with st.expander("Settings used for this snapshot"):
                            st.json(selected_snap_details.get("settings_used", {}))
                except FileNotFoundError: # Should be caught by os.path.exists, but as a fallback
                    st.error(f"Snapshot image file not found: {selected_snap_details['image_path']}.")
                except Exception as e:
