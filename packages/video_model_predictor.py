...
from packages.image_model_predictor import ImageModelPredictor # To reuse single frame prediction logic
...
class VideoModelPredictor:
    def __init__(self, bbox_model_loader: BoundingBoxModelLoader, seg_model_loader: SegmentationModelLoader):
        ...
        self.image_predictor = ImageModelPredictor(bbox_model_loader, seg_model_loader)
    ...
    def process_video(self, video_path: str, frame_interval: int, detection_threshold: float, defect_types: list, progress_callback=None, output_dir: str = None, save_processed_video: bool = False):
        ...
        if output_dir and not os.path.exists(output_dir):
            ...
            print(f"[DEBUG] Created output directory for video processing: {output_dir}")

        if save_processed_video and output_dir:
            base_name = os.path.basename(video_path)
            name, ext = os.path.splitext(base_name)
            processed_video_filename = f"processed_{name}_{int(time.time())}.mp4" # Use mp4 for wider compatibility
            ...
            print(f"[DEBUG] Initialized video writer for: {processed_video_path}")
        ...
        while cap.isOpened():
            ...
            if frame_number % frame_interval == 0:
                ...
                print(f"[DEBUG] Processing frame {frame_number} (Timestamp: {timestamp_ms:.2f}ms)")

                # Use the image_predictor to process the frame
                processed_frame_bgr, frame_detections = self.image_predictor.predict_on_image(frame_bgr.copy()) # Send a copy

                # Filter detections based on threshold and type
                filtered_detections = []
                if frame_detections:
                    for det in frame_detections:
                        conf = det.get('confidence', 0.0) # Default to 0 if no confidence
                ...
                if filtered_detections:
                    all_detections_by_frame.append({
                        ...
                        "original_frame_path": None, # Could be added if frames are saved individually pre-analysis
                        "processed_frame_path": None # Will be updated if frame is saved
                    })
                    print(f"[DEBUG] Frame {frame_number}: Found {len(filtered_detections)} defects after filtering.")

                # Save individual processed frame if output_dir is provided (optional, can be controlled by another param)
                # For now, let's assume we save frames if output_dir is given, but not make it a primary feature
                # for the video itself, which will be written by processed_video_writer
                if output_dir and not save_processed_video: # Example: save some keyframes if not saving full video
                    if filtered_detections or (processed_frame_count < 5): # Save if detections or one of first 5 processed
                        ...
                        try:
                            ...
                            print(f"[DEBUG] Saved processed frame: {frame_save_path}")
                        except Exception as e:
                            print(f"[ERROR] Could not save frame {frame_save_path}: {e}")
        ...
        if processed_video_writer:
            ...
            print(f"[DEBUG] Released video writer. Processed video saved to: {processed_video_path}")
        ...
        print(f"[INFO] Video processing complete. Total frames processed: {processed_frame_count}. Detections logged for {len(all_detections_by_frame)} frames.")
